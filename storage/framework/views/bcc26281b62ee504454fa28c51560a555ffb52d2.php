			...
			<table class="table table-striped- table-bordered table-hover table-checkable" id="dtable">
				...
				<tbody>
					...
					<tr>
						...
						<td><?php echo e($gv->sum); ?>р. (счет: <?php echo e($gv->type == 'balance' ? 'Реальный' : 'Бонусный'); ?>)</td>
						<td><?php echo e(\Carbon\Carbon::parse($gv->time_to)->setTimezone('Europe/Moscow')->format('d.m.Y H:i:s')); ?></td>
						<td><?php if($gv->winner_id): ?><a href="/admin/user/<?php echo e($gv->winner_id); ?>"><?php endif; ?><?php echo e(($gv->winner_id ? \App\User::getUser($gv->winner_id)->username : 'Не определен')); ?><?php if($gv->winner_id): ?></a><?php endif; ?></td>
						<td><a class="btn btn-sm btn-clean btn-icon btn-icon-md" title="Редактировать" data-toggle="modal" href="#edit_<?php echo e($gv->id); ?>"><i class="la la-edit"></i></a><a href="/admin/giveaway/delete/<?php echo e($gv->id); ?>" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="Удалить"><i class="la la-trash"></i></a></td>
					</tr>
