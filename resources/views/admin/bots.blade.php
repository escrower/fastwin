			...
			<table class="table table-striped- table-bordered table-hover table-checkable" id="dtable">
				...
				<tbody>
					...
					<tr>
						...
						<td>{{ $bot->time == 0 ? 'Все время' : '' }}{{ $bot->time == 1 ? 'Утром (с 6ч до 12ч)' : '' }}{{ $bot->time == 2 ? 'Днем (с 12ч до 18ч)' : '' }}{{ $bot->time == 3 ? 'Вечером (с 18ч до 00ч)' : '' }}{{ $bot->time == 4 ? 'Ночью (с 00ч до 6ч)' : '' }}</td>
						<td><a href="https://vk.com/id{{$bot->user_id}}" target="_blank">Перейти</a></td>
						<td><a href="/admin/user/{{$bot->id}}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="Редактировать"><i class="la la-edit"></i></a><a href="/admin/bots/delete/{{$bot->id}}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="Удалить"><i class="la la-trash"></i></a></td>
					</tr>
					@endforeach
				</tbody>
			</table>
