	...
	public function createRoom(Request $r) {
		...
		try {
			...
			if($r->get('sum') < $this->settings->flip_min_bet) {
				DB::rollback();
				return response()->json(['type' => 'error', 'msg' => 'Минимальная ставка '.$this->settings->flip_min_bet.' руб.!']);
			}
			if($r->get('sum') > $this->settings->flip_max_bet) {
				DB::rollback();
				return response()->json(['type' => 'error', 'msg' => 'Максимальная ставка ставка '.$this->settings->flip_max_bet.' руб.!']);
			}
	...
	public function joinGame(Request $r) {
		...
		try {
			...
			if($balType != $room->balType) {
				DB::rollback();
				return response()->json(['type' => 'error', 'msg' => 'В данной комнате учавствует только '. (($balType == 'balance') ? 'бонусный' : 'реальный') .' счет!']);
			}
			if($room->status == 1) {
				DB::rollback();
