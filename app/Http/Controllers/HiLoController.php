...
class HiLoController extends Controller
{
	...
	public function newBet(Request $r) {
		...
		if($this->game->status > 1) return response()->json(['msg' => 'Ставки в эту игру закрыты!', 'type' => 'error']);
		if($factor == 'error') return response()->json(['msg' => 'Не удалось определить тип ставки', 'type' => 'error']);
		if(is_null($sum)) return response()->json(['msg' => 'Вы ввели неверную сумму', 'type' => 'error']);
		if($sum < $this->settings->hilo_min_bet) return response()->json(['msg' => 'Минимальная сумма ставки '.$this->settings->hilo_min_bet.'!', 'type' => 'error']);
		if($sum > $this->settings->hilo_max_bet) return response()->json(['msg' => 'Максимальная сумма ставки '.$this->settings->hilo_max_bet.'!', 'type' => 'error']);
		if($balance == 'balance' && $this->user->balance < $sum) return response()->json(['type' => 'error', 'msg' => 'Недостаточно баланса!']);
		if($balance == 'bonus' && $this->user->bonus < $sum) return response()->json(['type' => 'error', 'msg' => 'Недостаточно баланса!']);
		if($factor[2] == 0) return response()->json(['msg' => 'Вы не можете сделать ставку на данный множитель!', 'type' => 'error']);
		
		$countbets = HiLoBets::where('game_id', $this->game->id)->where('user_id', $this->user->id)->count();
		if($countbets >= $this->settings->hilo_bets) return response()->json(['msg' => 'Вы сделали максимальное количество ставок!', 'type' => 'error']);
	...
	private function sendMoney($game_id, $amount, $section, $sign) {
		$small = 0; $big = 0; $ka = 0; $a = 0; $red = 0; $black = 0; $hi = 0; $lo = 0; $joker = 0;
